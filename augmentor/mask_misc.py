...
def rotate(image, annotations, prob=0.5, border_value=(128, 128, 128)):
    ...
    assert 'masks' in annotations, 'annotations must contain masks'
    ...
    # Rotate the masks.
    masks = annotations['masks'].astype(np.float32)
    if masks.shape[0] != 0:
        new_masks = []
        for mask in masks:
            mask = cv2.warpAffine(mask, M=M, dsize=(new_w, new_h), flags=cv2.INTER_CUBIC,
                           borderMode=cv2.BORDER_CONSTANT,
                           borderValue=border_value)
...
def crop(image, annotations, prob=0.5):
    ...
    if bboxes.shape[0] != 0:
        ...
        for mask in annotations['masks']:
            #new_mask = np.zeros((random_y1-random_y2, random_x2-random_x1), dtype=bool)
            new_mask = mask[random_y1:random_y2, random_x1:random_x2]
            new_masks.append(new_mask)
        annotations['masks'] = np.array(new_masks)
    else:
...
def multi_scale(image, annotations, prob=1.):
    ...
    bboxes = annotations['bboxes']
    if bboxes.shape[0] != 0:
        annotations['masks'] = np.array([cv2.resize(mask, (nh, nw), interpolation=cv2.INTER_LINEAR) 
                                            for mask in annotations['masks']], dtype=bool)
...
def translate(image, annotations, prob=0.5, border_value=(128, 128, 128)):
    ...
    if bboxes.shape[0] != 0:
        ...
        for mask in masks:
            ...
            new_masks.append(mask)
        annotations['masks'] = np.array(new_masks).astype(bool)
...
if __name__ == '__main__':
    ...
    for i in range(train_generator.size()):
        ...
        image = image.copy()
        boxes = annotations['bboxes'].astype(np.int32)
        masks = annotations['masks'].astype(bool)
        #quadrangles = annotations['quadrangles'].astype(np.int32)
        for box in boxes:
        ...
        for i,mask in enumerate(masks):
            cv2.imwrite('test_effects/mask_{}'.format(i), mask)
            cv2.imread('test_effects/mask_{}.jpg'.format(i))
        cv2.imwrite('test_effects/image.jpg', image)
        cv2.imread('test_effects/image.jpg')
