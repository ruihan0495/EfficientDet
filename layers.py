...
from utils.graph_funcs import batch_slice
...
def refine_detections_graph(rois, probs, detection_min_conf,
                            detection_max_instance, detection_nms_threshold):
    ...
    if detection_min_conf:
        conf_keep = tf.where(class_scores >= detection_min_conf)[:, 0]
    ...
    def nms_keep_map(class_id):
        ...
        gap = detection_max_instance - tf.shape(class_keep)[0]
        class_keep = tf.pad(class_keep, [(0, gap)],
                            mode='CONSTANT', constant_values=-1)
        # Set shape so map_fn() can infer result shape
        class_keep.set_shape([detection_max_instance])
        return class_keep
    ...
    keep = tf.sparse_tensor_to_dense(keep)[0]
    # Keep top detections
    roi_count = detection_max_instance
    ...
    gap = detection_max_instance - tf.shape(detections)[0]
...
class DetectionLayer(keras.layers.Layer):
    ...
    def __init__(self, min_conf, max_instance, nms_threshold, batch_size=1, **kwargs):
    ...
    def call(self, inputs):
        ...
        print("score shape", mrcnn_class.shape)

        # Run detection refinement graph on each item in the batch
        detections_batch = batch_slice(
            [rois, mrcnn_class],
            lambda x, y: refine_detections_graph(x, y, self.min_conf, self.max_instance, self.nms_threshold),
            1)
        ...
        return tf.reshape(
            detections_batch,
            [self.batch_size, self.max_instance, 6])

    def compute_output_shape(self, input_shape):
        return (None, self.max_instance, 6)
